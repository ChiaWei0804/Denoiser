...
class Trainer(object):
    def __init__(self,
        ...
        # old_model=th.load('best.pt.tar', map_location=device)  # 從頭訓練不需要
    ...
    def create_optimizer(self, optimizer, kwargs, state=None):
        ...
        # params_to_optimize = filter(lambda p: p.requires_grad, self.nnet.parameters())
    ...
    def train(self, data_loader):
        ...
        pbar = tqdm(data_loader, desc=f"Train Epoch {self.cur_epoch}", leave=True)
        for egs in pbar:
            # load to gpu
            ...
            if self.clip_norm:
                ...
                clip_grad_norm_(self.trainable_params, self.clip_norm)
    ...
    def eval(self, data_loader):
        ...
        with th.no_grad():
            pbar = tqdm(data_loader, desc="Eval", leave=True)
            for egs in pbar:
                egs = load_obj(egs, self.device)
                with autocast(device_type="cuda"):
                    loss = self.compute_loss(egs)
    ...
    def run(self, train_loader, dev_loader, num_epochs=50):
        ...
        with th.cuda.device(self.gpuid[0]):
            ...
            while self.cur_epoch < num_epochs:
                self.cur_epoch += 1
                # ── τ annealing for Gumbel-Softmax (方向 C) ──
                ...
                print(f"  Epoch {self.cur_epoch} Summary")
                print(f"  Train Loss: {tr['loss']:+.4f}  |  Dev Loss: {cv['loss']:+.4f}")
            ...
            print(train_loss_lst, dev_loss_lst)
            
        plot(train_loss_lst, dev_loss_lst, num_epochs)
...
class SiSnrTrainer(Trainer):
    ...
    def compute_loss(self, egs):
        ...
        def sisnr_loss(permute):
            ...
            # return self.sisnr(ests[permute], refs[1])
        ...
        if self.loss_mode == "snr":
            ...
            # sisnr_mat = th.stack(
            #     [sisnr_loss(p) for p in range(num_spks)])
        else:
            raise ValueError("must specify loss_mode to sisnr or snr")
        max_perutt, _ = th.max(sisnr_mat, dim=0)
        # si-snr
        return -th.sum(max_perutt) / N

def plot(train_loss_lst, dev_loss_lst, num_epochs):
    actual_epochs = len(train_loss_lst)
    if actual_epochs == 0:
        return
    plt.figure(figsize=(10, 6))
    plt.plot(range(1, actual_epochs + 1), train_loss_lst, label='Train', color='red')
    plt.plot(range(1, actual_epochs + 1), dev_loss_lst, label='Dev', color='blue')
