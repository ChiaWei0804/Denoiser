...
class MS_SL2_split_model(nn.Module):
    def __init__(self,
        ...
        # 方向 C: Phase Shift — Group(512→256) + FC(256→4), 額外參數: 256×4+4 = 1,028
        self.fc_shift = nn.Linear(N // 2, 4)   # D=256
    ...
    def forward(self, x):
        ...
        for Slice in range(self.slice):
            ...
            # 方向 A: 用 Slice 的 skip_connection 做 pooling
            c_skip = skip_connection.mean(dim=-1)              # (B, Sc=256)
            alpha = th.sigmoid(self.fc_scale(c_skip))          # (B, 4)
            
            # 方向 C: 用 Slice 的 skip_connection 做 Gumbel-Softmax
